...
- 1페이지는 `UI/QnA.png` 기반 PyQt5 설문 화면으로 구성하고, 2페이지는 `UI/TypeA.png` 기반 자세 측정 화면으로 이어진다.
- 설문 페이지에는 8개 체질/성향 질문 섹션, 선택 체크박스, `설문완료` 버튼, 좌측 슬라이드 전환 애니메이션을 적용했다.
...
- 키보드 입력으로 `Q` 종료, `R` 측정값/혀 미리보기 초기화, `F` 좌우 반전, `O` 화면 회전을 지원하고 `Ctrl + C` 종료도 처리한다.
...
- `BodyCheck.py`: 설문 페이지(PyQt5), 페이지 전환 애니메이션, 실시간 자세 측정, 카메라 처리, 얼굴/혀 미리보기, UI 합성
