...
- 시작 화면은 `UI/Main.png` 기반 PyQt5 메인 페이지로 구성하고, 우측 하단 로고 클릭 시 좌측 슬라이드 애니메이션으로 `UI/QnA.png` 설문 페이지로 이동한다.
- 설문 페이지에는 8개 체질/성향 질문 섹션, 선택 체크박스, `설문완료` 버튼, 좌측 슬라이드 전환 애니메이션을 적용했고, 텍스트와 체크 표시를 흰색 계열로 정리했다.
- 설문 완료 후에는 `UI/TypeA.png` 기반 자세 측정 페이지로 같은 창 안에서 자연스럽게 이어진다.
- 2페이지는 같은 PyQt5 전체화면 창 안에서 유지되며, 설문 완료 후 창이 꺼지지 않고 바로 자세 측정 화면으로 이어진다.
- BodyCheck 측정 화면은 본체 카메라, 얼굴 미리보기, 혀 미리보기 영역을 UI에 맞춰 합성한다.
- MediaPipe Pose를 사용해 어깨 기울기, 골반/허리 기울기, 상체 기울기, 중심 이동 값을 계산한다.
- 여러 프레임 평균값을 사용해 자세 지표를 안정적으로 표시한다.
- 얼굴 프리뷰는 실선 박스를 꽉 채우도록 비율 보정 후 합성하고, 혀 프리뷰는 입 영역 중심으로 실시간 갱신되도록 조정했다.
- 혀 인식 로직은 HSV/LAB/BGR 조건과 입 하단 중심부 비율을 함께 사용하도록 보강했다.
- 2페이지 오버레이 텍스트는 흰색 계열로 조정했고, 키보드 입력으로 `Q` 종료, `R` 측정값/혀 미리보기 초기화, `F` 좌우 반전, `O` 화면 회전을 지원하며 터미널 `Ctrl + C` 종료도 처리한다.
...
- `BodyCheck.py`: 메인/설문/측정 페이지(PyQt5), 로고 기반 슬라이드 전환, 실시간 자세 측정, 카메라 처리, 얼굴/혀 미리보기, UI 합성
...
- `UI/Main.png`: BodyCheck 시작 메인 화면 배경 이미지
- `UI/QnA.png`: BodyCheck 1페이지 설문 배경 이미지
- `UI/TypeA.png`: BodyCheck 2페이지 측정 배경 UI 이미지
...
- 2026-03-18: `BodyCheck.py`에 1페이지 설문 UI(`QnA.png`)와 2페이지 측정 UI(`TypeA.png`)를 연결하고, 설문 체크박스/완료 버튼/슬라이드 전환을 추가
- 2026-03-18: 2페이지를 별도 OpenCV 창이 아닌 동일 PyQt5 전체화면 창에서 유지하도록 구조를 재구성
- 2026-03-18: 얼굴 프리뷰 비율 보정, 혀 인식 로직 개선, 혀 프리뷰 실시간 갱신, 로딩/전환 흐름 정리
- 2026-03-18: `UI/Main.png`를 BodyCheck 첫 화면으로 추가하고, 우측 하단 로고 클릭 시 `QnA -> TypeA` 흐름으로 이어지는 슬라이드 전환과 `Ctrl + C` 종료 처리를 보강
- 2026-03-18: `QnA.png` 설문 페이지의 텍스트/체크 표시와 `TypeA.png` 측정 페이지 오버레이 텍스트를 흰색 계열로 정리하고, 관련 UI 이미지(`UI/QnA.png`, `UI/TypeA.png`)를 최신 레이아웃으로 갱신
